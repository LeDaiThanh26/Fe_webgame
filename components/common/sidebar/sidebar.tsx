...
export default function Sidebar({ isMobile, isSearchOpen, setIsSearchOpen, isProfile = false, setIsProfile }: SidebarProps) {
  ...
  return (
    <>
      {isSearchOpen && <div className="fixed inset-0 bg-black/60 z-[90]" onClick={closeSidebar} />}
      <div className={`fixed top-0 left-0 h-full ${isProfile ? "bg-[#FFC0CB]" : "bg-[#00ffee]"} shadow-2xl z-[100]
        ...
        <div className="flex flex-col h-full p-6 overflow-y-auto no-scrollbar">
          ...
          {isProfile
            ? <SidebarProfile user={user} avatarUrl={avatarUrl} totalGame={totalGame} />
            : <SidebarSearch searchGames={searchGames} recommendedGames={recommendedGames} recentGames={recentGames} favouriteGames={favouriteGames} />
          }
        </div>
      </div>
    </>
