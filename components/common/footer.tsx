...
export default function Footer() {
    ...
    if (windowWidth > 2300) centerPx = windowWidth * 0.45;
    ...
    return (
        <footer className={`flex items-end justify-center w-full ${isMobile ? "h-[570px]" : "h-[457px]"} relative bg-[#FFFFFF] overflow-hidden`}
            style={{ clipPath: `polygon(${isMobile ? `0% 10%, 95% 0%, 100% 5%, 100% 100%, 0% 100%` : `0% 25%, ${pos1} 20%, ${pos2} 25%, 100% 0%, 100% 100%, 0% 100%`})` }}>
            <div className="absolute top-0 left-0 w-full h-full z-0"
                style={{ background: "#F0F5FC", clipPath: `polygon(${isMobile ? `95% 0%, 100% 5%, 100% 100%, 90% 100%` : `${pos1} 20%, ${pos2} 25%, ${pos3} 100%, ${pos4} 100%`})` }} />
            <div className={`flex ${isMobile ? "flex-col gap-10" : "justify-between"} mx-[20px] w-full max-w-[1860px] pb-15 z-10 text-[#002B50] relative`}>
                <div className={`flex flex-col gap-4 ${isMobile ? "w-[800px]" : "w-[400px] ml-20"}`}>
                    <div className="flex items-center gap-2">
                        <Image src="/icon5.png" alt="logo" width={120} height={120} className="object-contain" />
                    ...
                    <p className={`font-medium ${isMobile ? "text-sm" : ""}`}>
                        GameZone mang đến cho bạn hàng ngàn trò chơi trực tuyến hấp dẫn, đa dạng thể loại như phiêu lưu, trí tuệ, hành động, thể thao, và giải trí nhẹ nhàng. Chơi ngay không cần tải xuống!
                    </p>
                ...
                <div className="flex w-[600px] gap-40">
                    <div className="flex flex-col gap-2.5">
                        ...
                        {[["Trang chủ", "/"], ["Danh mục game", "/category"], ["Liên hệ", "#"]].map(([label, href]) => (
                            <a key={label} className={`${isMobile ? "text-sm" : "text-[17px]"} font-medium hover:text-blue-600 transition-colors`} href={href}>{label}</a>
                        ))}
                    </div>
                    <div className="flex flex-col gap-2.5">
                        <h3 className={`font-bold ${isMobile ? "text-base" : "text-lg"}`}>Chính sách & Hỗ trợ</h3>
                        {[["Điều khoản sử dụng", "#"], ["Chính sách bảo mật", "#"], ["Báo lỗi game", "#"]].map(([label, href]) => (
                            <a key={label} className={`${isMobile ? "text-sm" : "text-[17px]"} font-medium hover:text-blue-600 transition-colors`} href={href}>{label}</a>
                        ))}
                    </div>
                </div>
