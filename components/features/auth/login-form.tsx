...
const LoginForm: React.FC<LoginFormProps> = ({ onClose, onToggleMode }) => {
  ...
  return (
    <div className="relative w-full max-w-sm p-8 bg-teal-900/90 rounded-2xl shadow-2xl border-2 border-teal-700/50">
      ...
      <h2 className="text-white text-2xl font-bold text-center mt-4 mb-8">Đăng nhập tại đây</h2>
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <input name="email" type="email" placeholder="Điền vào email"
        ...
        <div className="mb-4">
          <input name="password" type="password" placeholder="Điền vào mật khẩu"
            className="w-full px-4 py-3 bg-gray-900/70 text-white placeholder-gray-500 rounded-xl border border-teal-700 focus:outline-none focus:ring-1 focus:ring-teal-400"
            required onChange={handleChange} />
        </div>
        ...
        <p className="text-center text-sm mb-8 text-gray-300">
          <a href="#" className="text-red-500 font-medium hover:text-red-400"
            onClick={(e) => { e.preventDefault(); onToggleMode(); }}>Đăng ký</a>{" "}tại đây nếu chưa có tài khoản
        </p>
