...
import GameTable from "@/components/features/admin/GameTable";
import GameFormModal from "@/components/features/admin/GameFormModal";
...
const INITIAL_FORM = {
  ...
  platforms: "", thumbnail: "", image: "", link_game: "", video: "", category: "",
...
export default function AdminDashboard() {
  ...
  const handleDelete = async (slug: string) => {
    ...
    const res = await api.delete(`/games/${slug}`, { Authorization: `Bearer ${getToken()}` });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const payload = { ...formData, platforms: platformsArr, image: images, thumbnail: formData.thumbnail || images[0] || undefined, release_date: formData.release_date ? new Date(formData.release_date).toISOString() : undefined };
    const authHeader = { Authorization: `Bearer ${getToken()}` };
    const isEdit = !!currentGame;
    const res = isEdit ? await api.put(`/games/${currentGame!.slug}`, payload, authHeader) : await api.post("/games", payload, authHeader);
    if (res.ok) {
      const body = await res.json();
      const data = body.data ?? body;
      setGames((prev) => isEdit ? prev.map((g) => (g._id === data._id ? data : g)) : [...prev, data]);
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="relative flex-1 max-w-md w-full">
            ...
            <input type="text" placeholder="Tìm kiếm game..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0095BE]" />
          </div>
      ...
      {showModal && <GameFormModal currentGame={currentGame} formData={formData} updateField={updateField} onSubmit={handleSubmit} onClose={() => setShowModal(false)} />}
    </>
  );
}