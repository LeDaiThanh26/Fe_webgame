import CommentsSection from "@/app/ui/Detail/CommentsSection";
import SimilarGames from "@/app/ui/Detail/SimilarGames";
import Favourites from "@/app/ui/Detail/Favourite";
import GamePlayTracker from "@/app/ui/GamePlayTracker";
import { fetchGameBySlug } from "@/app/lib/api/games";
...
const GameDetails = async ({ params }: { params: { slug: string } }) => {
  ...
  const cssStyles = `
    :root { --primary-bg: #f0f2f5; --white: #ffffff; --text-color: #333; --text-light: #666; --border-color: #e5e5e5; }
    ...
    .btn-love { border: 1px solid #ddd; background: white; padding: 8px 20px; border-radius: 20px; cursor: pointer; font-size: 14px; display: flex; align-items: center; gap: 8px; font-weight: 600; transition: all 0.3s ease; color: #333; }
    .btn-love:hover { background-color: #fff1f1; border-color: #ff4d4d; color: #ff4d4d; }
    .btn-love.active { background-color: #ff4d4d; color: white; border-color: #ff4d4d; box-shadow: 0 2px 8px rgba(255,77,77,0.4); }
    .btn-love.active:hover { background-color: #e60000; }
    ...
    .game-image-container { width: 100%; border-radius: 8px; margin-bottom: 20px; display: flex; flex-direction: column; gap: 10px; }
    .game-image-container img { width: 100%; height: auto; max-height: 420px; object-fit: contain; display: block; }
        ...
        <div className="card">
          ...
          <table className="info-table">
            <tbody>
              ...
              <tr><td>Lần update cuối</td><td>{data.last_update ? new Date(data.last_update).toLocaleDateString("vi-VN") : "N/A"}</td></tr>
          ...
          <p className="note-text">
            {data.how_to_play || "Lưu ý: Khi chơi trên máy tính, bạn hãy chuyển chế độ gõ từ tiếng Việt sang tiếng Anh!"}
          </p>
